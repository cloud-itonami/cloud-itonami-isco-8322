(ns driving.actor
  "CarVanDrivingPracticeActor — the ISCO-08 8322 community driving actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one driving operation request
  ...
  The unconditional invariant: the CarVanDrivingPracticeAdvisor can never
  directly commit a record the CarVanDrivingPracticeGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [driving.advisor :as advisor]
            [driving.governor :as governor]
            [driving.store :as store]))

(defn build-graph
  "Build a compiled CarVanDrivingPracticeActor graph. `store` implements
  `driving.store/Store`. `advisor` implements
  `driving.advisor/Advisor` (defaults to `mock-advisor`).
